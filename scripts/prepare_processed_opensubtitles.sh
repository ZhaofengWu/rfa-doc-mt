...
if [[ ${#} -ne 3 && ${#} -ne 5 ]]; then
  echo "usage: bash scripts/prepare_processed_opensubtitles.sh DATASET_DIR SRC_LANG TGT_LANG (START_STAGE END_STAGE)"
  exit 1
fi
...
tgt=$3
pair=$src-$tgt
output_dir=data/opensubtitles/${pair}
...
if [[ ${start_stage} -le 1 && ${end_stage} -ge 1 ]]; then
  ...
  time python scripts/parse_processed_opensubtitles.py ${dataset_dir} ${intermediate_dir}
fi
...
if [[ ${start_stage} -le 8 && ${end_stage} -ge 8 ]]; then
    ...
    cp ${intermediate_dir}/*.doc_end_indices ${output_dir}
    cp ${intermediate_dir}/*.doc_end_indices ${output_dir}/binary
fi
